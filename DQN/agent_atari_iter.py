...
class DQN_Agent():
    ...
    def train(self):
        ...
        for i in range(int(self.args.iter)):
            state_tensor= torch.tensor(state, dtype= torch.float32)
            if args.cuda:
                state_tensor= state_tensor.cuda()
            # select an action:
            action= self.select_action(state_tensor.unsqueeze(0), i)
            ...
            if len(epi_reward[-101: -1])==0:
                mean_reward_per_100= 0
            else:
                mean_reward_per_100= np.mean(epi_reward[-101:-1])

            if i % self.args.display_interval==0:
                print('[{}], Iter: {}, Loss: {:.3f}, Reward:{:.2f}'. format(datetime.now(), i, loss, mean_reward_per_100))
